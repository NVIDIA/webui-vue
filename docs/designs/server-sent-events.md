...
events to browser clients. This design integrates SSE into WebUI-Vue using Vue 3
(Pinia, VueQuery and composables) to reduce polling where Redfish events exist.
...
- [Gerrit 87161](https://gerrit.openbmc.org/c/openbmc/webui-vue/+/87161)
  (this design, under review)
...
5. Support `$filter` for EventType, MessageId, RegistryPrefix, ResourceType, and
   OriginOfCondition
...
This feature is runtime optional. SSE is enabled only when authenticated and may
be disabled via configuration. All endpoints are standard Redfish.

## Proposed Design

The solution introduces a Pinia SSE store, composables to manage the
EventSource lifecycle and cache invalidation, and a shared event parsing
layer that normalizes Redfish Event payloads. Vue Query is used to
invalidate caches when events reference resources.
...
The SSE infrastructure is split across focused modules:

| File                                   | Purpose                                      |
|----------------------------------------|----------------------------------------------|
| `src/stores/sse.ts`                    | Pinia store — connection state, event buffer, reconnect config |
| `src/api/composables/useSSE.ts`        | EventSource lifecycle — open, reconnect with backoff, close    |
| `src/api/composables/useSSEInit.ts`    | Initialization — connects SSE to Vue Query, handles power/firmware events, toast notifications |
| `src/api/composables/useSSEQueryInvalidation.ts` | Cache invalidation — maps SSE events to Vue Query cache keys and triggers refetch |
| `src/api/composables/parseSSEEvent.ts` | Event parsing utilities — extracts `OriginOfCondition`, `ResourceType`, `MessageId` matching |

### Connection and auth

- SSE uses the existing authenticated session cookies.
- The endpoint requires the same privilege as POSTing a subscription.
- On auth expiry or 401/403 responses, the client stops reconnecting until the
  user logs in again.
...
- The parser (`parseSSEEvent.ts`) handles:
...
- Heartbeat events (`HeartbeatEvent.*`) are skipped by the cache
  invalidation engine — they are for connection health only.
- `EventBufferExceeded` triggers `queryClient.invalidateQueries()` to
  flush the entire Vue Query cache and refetch all active queries.

### Vue Query cache invalidation

The `useSSEQueryInvalidation` composable watches the SSE event stream and
maps events to Vue Query cache keys for automatic refetch.  Two mechanisms
work together (see `docs/designs/vue-query-integration.md` for full
details on the query key convention):

**Dynamic path-based invalidation** (primary): Extracts the
`OriginOfCondition` URI, strips the `/redfish/v1/` prefix, and
invalidates the matching query key.  For `ResourceCreated` /
`ResourceRemoved` events, the parent collection key is also invalidated.
...
**Static rule-based invalidation** (supplementary): Predefined rules match
`MessageId` patterns and `ResourceType` values to additional query keys.
For example, a `TaskEvent` invalidates both `['TaskService']` and
`['TaskService', 'Tasks']`.
...
- When SSE is disabled, unavailable, or repeatedly failing, the UI falls back to
  existing polling only, without introducing new polling paths. As SSE coverage
  grows, existing polling should be reduced or removed.
- UI state shows SSE connection status via a status indicator component
  (`SSEStatusIndicator.vue`) to avoid silent staleness.

### Implementation status

The core SSE infrastructure is implemented and functional:

- Pinia store (`sse.ts`): connection state, bounded event buffer, reconnect
  configuration with exponential backoff
- EventSource lifecycle (`useSSE.ts`): open/close, automatic reconnection,
  `Last-Event-Id` replay
- Vue Query cache invalidation (`useSSEQueryInvalidation.ts`): dynamic
  path-based + static rule-based invalidation with deduplication
- Event parsing (`parseSSEEvent.ts`): `OriginOfCondition` extraction,
  `ResourceType` parsing, `MessageId` pattern matching
- SSE initialization (`useSSEInit.ts`): firmware update task tracking,
  power state events, toast notifications for critical alerts
- Event log integration (`useEventLog.ts`): real-time log entry updates
  via SSE with incremental `$filter` fetching

Remaining work:

- MetricReport event handling for TelemetryService sensor updates
- localStorage backup of SSE store state across browser refresh
- Multi-tab coordination for subscription limit management

## Alternatives Considered

VueUse `useEventSource`: Rejected because it adds a new dependency for a small
feature and the project does not currently use VueUse.

WebSocket: Previously supported and [removed][ws-removal]. Rejected because
Redfish specifies SSE for EventService and SSE is simpler for server-to-client
events.

[ws-removal]: https://gerrit.openbmc.org/c/openbmc/webui-vue/+/70641

Aggressive polling: Rejected because it increases BMC load and still has latency
for critical alerts.

## Impacts

API impact: None. Uses existing Redfish endpoints and registries.

Security impact: Positive. SSE uses existing session cookies and bmcweb enforces
subscription limits and privileges.

Documentation impact: Adds an SSE usage guide and references to bmcweb event
semantics.

Performance impact: Lower polling load and reduced latency for critical events;
minimal client overhead.

Developer impact: Introduces shared SSE infrastructure and event normalization,
enabling simple integration for new views.

Upgradability impact: No vendor-specific extensions required; adheres to Redfish
EventService.
