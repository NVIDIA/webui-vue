...
// --- GENERATED PRIVILEGE_MAPPINGS START ---
...
// --- GENERATED PRIVILEGE_MAPPINGS END ---

/**
 * Hand-maintained privilege mappings for entities used via canCallEntity()
 * (dynamically discovered URIs) or entities not yet in the generated set.
 * These are merged into PRIVILEGE_MAPPINGS at load time.
 * Generated entries take precedence for overlapping keys.
 */
const MANUAL_PRIVILEGE_MAPPINGS: Record<string, Record<string, string[][]>> = {
  // Session management (used by canCallEntity in auth flows)
  Session: {
    GET: [['Login']],
...
// Merge manual mappings into generated ones (generated takes precedence)
for (const [entity, methods] of Object.entries(MANUAL_PRIVILEGE_MAPPINGS)) {
  if (!PRIVILEGE_MAPPINGS[entity]) {
    PRIVILEGE_MAPPINGS[entity] = methods;
  }
}
