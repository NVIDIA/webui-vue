...
export default {
  ...
  computed: {
    ...
    connectionStatusText() {
      switch (this.connectionState) {
        case ConnectionState.OPEN:
          return i18n.global.t('pageSerialOverLan.connected');
        case ConnectionState.CONNECTING:
          return i18n.global.t('pageSerialOverLan.connecting');
        case ConnectionState.ERROR:
          return i18n.global.t('pageSerialOverLan.connectionError');
        case ConnectionState.CLOSING:
          return i18n.global.t('pageSerialOverLan.disconnected');
        case ConnectionState.CLOSED:
        default:
          return i18n.global.t('pageSerialOverLan.disconnected');
      }
      ...
      try {
        ...
        this.ws.onclose = (event) => {
          ...
          if (event.code !== 1000) { // Normal closure
            this.connectionError = event.reason || i18n.global.t('pageSerialOverLan.connectionClosedUnexpectedly');
          }
        };
        this.ws.onerror = (error) => {
          console.log('websocket console/default error', error);
          this.connectionState = ConnectionState.ERROR;
          this.connectionError = i18n.global.t('pageSerialOverLan.failedToConnect');
        };
      } catch (error) {
        console.log(error);
        this.connectionState = ConnectionState.ERROR;
        this.connectionError = error.message || i18n.global.t('pageSerialOverLan.failedToConnect');
      }
    },
    closeTerminal() {
