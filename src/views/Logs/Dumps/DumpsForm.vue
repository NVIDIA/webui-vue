<template>
  <div class="form-background p-3">
    ...
    >
      ...
      >
        ...
        >
          ...
          <b-form-select-option v-for="option in dumpTypeOptions" :key="option.value" :value="option">
    ...
    <modal-confirmation ref="systemDumpModal" @ok="createSystemDump" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      try {
        if (this.selectedDumpType.type === DUMP_TYPES.SYSTEM) {
          ...
          const missing = this._getMissingRequiredParams();
          if (missing.length) {
            return;
          }
          ...
          this.$store
            ...
            .then(() => {
              this.infoToast(i18n.global.t('pageDumps.toast.successStartBmcDump'), {
    ...
    showConfirmationModal() {
      try {
        this.$bvModal && this.$bvModal.show('modal-confirmation');
      } catch (e) {
        console.error('[DumpsForm] failed to open modal', e);
      }
      try {
        const modalRef = this.$refs.systemDumpModal && this.$refs.systemDumpModal.$refs && this.$refs.systemDumpModal.$refs.modal;
        if (modalRef && typeof modalRef.show === 'function') {
          modalRef.show();
        }
      } catch (e) {
        console.error('[DumpsForm] fallback failed', e);
      }
    },
    ...
    createSystemDump() {
      ...
      const missing = this._getMissingRequiredParams();
      if (missing.length) {
        this.formError = i18n.global.t('global.form.required');
        return;
      }
      this.isSubmitting = true;
      this.formError = null;
      ...
      this.$store
        .dispatch('dumps/createDump', payload)
        .then(() => {
          this.infoToast(i18n.global.t('pageDumps.toast.successStartSystemDump'), {
    ...
    _getMissingRequiredParams() {
      try {
        const requiredParams = (this.selectedDumpType?.Parameters || []).filter((p) => p.Required);
        const missing = requiredParams
          .map((p) => ({ name: p.Name, value: this.parameterValues[p.Name] }))
