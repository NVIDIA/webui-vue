...
export const useFirmwareStore = defineStore('firmware', () => {
  ...
  const bmcFirmware = ref<FirmwareItem[]>([]);
  const hostFirmware = ref<FirmwareItem[]>([]);
  const firmwareInventory = ref<FirmwareItem[]>([]);
  const bmcActiveFirmwareId = ref<string | null>(null);
  const hostActiveFirmwareId = ref<string | null>(null);
  const bmcSoftwareImageIds = ref<string[]>([]);
  const hostSoftwareImageIds = ref<string[]>([]);
  ...
  const activeHostFirmware = computed(() =>
    hostFirmware.value.find((fw) => fw.id === hostActiveFirmwareId.value),
  );

  const backupBmcFirmware = computed(() =>
    bmcFirmware.value.filter((fw) => fw.id !== bmcActiveFirmwareId.value),
  );

  const backupHostFirmware = computed(() =>
    hostFirmware.value.filter((fw) => fw.id !== hostActiveFirmwareId.value),
  );
  ...
  async function getUpdateServiceSettings(): Promise<void> {
    try {
      const data = await getUpdateService();
